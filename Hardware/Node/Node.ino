...
#include <math.h>
#include "ACS712.h"
#define SSR_PIN_WRITE 2         /*SSR control pin 同時用LED顯示是否過電*/
#define BUTTON_PIN_READ 3          /*Button press*/
///////////////////////////////////////////////////////////////////
// CHANGE HERE THE READ PIN AND THE POWER PIN FOR THE TEMP. SENSOR
#define CURRENT_PIN_READ  A0
#define VOLTAGE_PIN_READ  A1
ACS712 sensor(ACS712_20A, CURRENT_PIN_READ);  /*電流感測器輸出腳位為 A0*/
...
#endif
#endif
unsigned int nCycle = idlePeriodInSec/LOW_POWER_PERIOD;
#endif
...
void setup()
{
  ...
  pinMode(SSR_PIN_WRITE, OUTPUT);
  digitalWrite(SSR_PIN_WRITE, HIGH);  /*先設定常開，之後要改*/
  pinMode(BUTTON_PIN_READ, INPUT);
  digitalWrite(BUTTON_PIN_READ, LOW);
  pinMode(CURRENT_PIN_READ, INPUT);
...
int getVoltageAC(){
  int sensor_Value = analogRead(VOLTAGE_PIN_READ);
...
void loop(void)
{
  ...
  if(!digitalRead(BUTTON_PIN_READ)){ /*Press button?*/
    Serial.println("XDD");}
  ...
  if (millis() > nextTransmissionTime) {
      ...
      I=0;
      V=0;
      Serial.println(digitalRead(SSR_PIN_WRITE));
      if(digitalRead(SSR_PIN_WRITE)){ /*有電流通過*/
        I = sensor.getCurrentAC();
        V = getVoltageAC();
      }else{              /*無電流通過*/
        I = 0;
        V = 0;}
      P+=V*I*0.24;//pow(2,-0.5)
      Serial.println(String("I = ") + I + " A");
      Serial.println(String("V = ") + V + " V");
      Serial.println(String("P = ") + P + " Watts");
      Serial.println(String("XDDDDDDDDDDDD"));
      
#ifdef LOW_POWER
      digitalWrite(SENSOR_PIN_POWER,LOW);
#endif
      ...
      nextTransmissionTime=millis()+(unsigned long)idlePeriodInSec*1000+(unsigned long)random(15,60)*1000;
      PRINT_VALUE("%ld", nextTransmissionTime);
      PRINTLN;
  }
#endif
